...
if df_filtered_global.empty:
    st.info("ℹ️ No hay datos que coincidan con los filtros seleccionados en el Tablero Principal. Regresa y ajusta la selección.")
    st.stop()
...
tab_traslados, tab_compras, tab_seguimiento = st.tabs(["🔄 Traslados Sugeridos", "🛒 Compras Sugeridas", "✅ Seguimiento de Órdenes"])
...
with tab_traslados:
    ...
    if df_plan_maestro is None or df_plan_maestro.empty:
        st.success("✅ ¡No se sugieren traslados automáticos en este momento! Buen trabajo de balanceo.")
    else:
        st.markdown("##### Filtra y ajusta el plan de traslados sugerido:")
        
        # UI de Filtros Mejorada para Traslados
        filter_cols = st.columns([2, 2, 3])
        df_aplicar_filtros = df_plan_maestro.copy()

        with filter_cols[0]:
            lista_origenes = ["Todos"] + sorted(df_plan_maestro['Tienda Origen'].unique().tolist())
            filtro_origen = st.selectbox("Filtrar por Tienda Origen:", lista_origenes, key="filtro_origen")
            if filtro_origen != "Todos":
                df_aplicar_filtros = df_aplicar_filtros[df_aplicar_filtros['Tienda Origen'] == filtro_origen]
        
        with filter_cols[1]:
            if not df_aplicar_filtros.empty:
                lista_destinos = ["Todas"] + sorted(df_aplicar_filtros['Tienda Destino'].unique().tolist())
                filtro_destino = st.selectbox("Filtrar por Tienda Destino:", lista_destinos, key="filtro_destino")
                if filtro_destino != "Todos":
                    df_aplicar_filtros = df_aplicar_filtros[df_aplicar_filtros['Tienda Destino'] == filtro_destino]

        with filter_cols[2]:
            search_term_traslado = st.text_input("Buscar por SKU o Descripción:", key="search_traslados", placeholder="Ej: 10501 o estuco")
            if search_term_traslado:
                mask_traslado = (df_aplicar_filtros['SKU'].astype(str).str.contains(search_term_traslado, case=False, na=False) |
                                 df_aplicar_filtros['Descripcion'].astype(str).str.contains(search_term_traslado, case=False, na=False))
                df_aplicar_filtros = df_aplicar_filtros[mask_traslado]

        st.markdown("---")

        if df_aplicar_filtros.empty:
...
# ==============================================================================
with tab_compras:
    st.header("🛒 Plan de Compras Sugeridas")
    ...
    if df_plan_compras.empty:
        ...
        st.markdown("##### Filtra las sugerencias para generar una orden de compra:")
        
        # UI de Filtros Mejorada para Compras
        proveedores_disponibles = ["Todos"] + sorted(df_plan_compras['Proveedor'].astype(str).str.upper().unique().tolist())
        ...
        if selected_proveedor != 'Todos':
            df_a_mostrar = df_a_mostrar[df_a_mostrar['Proveedor'].astype(str).str.upper() == selected_proveedor]
        
        st.markdown("---")
        
        if df_a_mostrar.empty:
            st.warning("No hay sugerencias para el proveedor seleccionado.")
        else:
...
with tab_seguimiento:
    ...
    if df_ordenes_historico.empty:
        ...
        df_ordenes_historico['Fecha_Emision'] = pd.to_datetime(df_ordenes_historico['Fecha_Emision'], errors='coerce').dt.date
        ...
        with st.expander("🔄 Cambiar Estado de Múltiples Órdenes (En Lote)", expanded=False):
            ...
            if not all(col in df_ordenes_vista_original.columns for col in required_filter_cols):
                ...
                with track_c1:
                    ...
                    filtro_estado = st.selectbox("Estado:", estados_disponibles, index=0, key="filtro_estado_seguimiento")
                ...
                with track_c2:
                    if not df_ordenes_vista.empty:
                        ...
                        filtro_proveedor_orden = st.selectbox("Proveedor/Origen:", proveedores_ordenes, key="filtro_proveedor_seguimiento")
                ...
                with track_c3:
                    if not df_ordenes_vista.empty:
                        ...
                        filtro_tienda_orden = st.selectbox("Tienda Destino:", tiendas_ordenes, key="filtro_tienda_seguimiento")
                ...
                if df_ordenes_vista.empty:
                    ...
                    if not df_seleccion_seguimiento.empty:
                        ...
                        if st.button(f"➡️ Actualizar {len(df_seleccion_seguimiento)} SKUs a '{nuevo_estado}'", key="btn_actualizar_estado", type="primary"):
                            ...
                            df_historico_modificado.reset_index(inplace=True)
                            ids_a_actualizar = df_seleccion_seguimiento['ID_Orden'].tolist()
                            skus_a_actualizar = df_seleccion_seguimiento['SKU'].tolist()
                            
                            mask = (df_historico_modificado['ID_Orden'].isin(ids_a_actualizar)) & (df_historico_modificado['SKU'].isin(skus_a_actualizar))
                            df_historico_modificado.loc[mask, 'Estado'] = nuevo_estado
                            df_historico_modificado.drop(columns=['index'], inplace=True)
                            ...
                            with st.spinner("Actualizando estados en Google Sheets..."):
                                ...
                                if exito:
                                    st.success(f"¡Éxito! {len(df_seleccion_seguimiento)} líneas de orden actualizadas. La página se recargará.")
                                    time.sleep(3)
                                    st.cache_data.clear()
                                    st.rerun()
                                else:
        ...
        with st.expander("🔍 Gestionar o Modificar una Orden Específica", expanded=True):
            ...
            if st.button("Cargar Orden", key="btn_load_order"):
                if orden_a_buscar:
                    ...
                    if not df_orden_cargada.empty:
                        ...
                        # Asegurar que se cargue el ID principal, no los que tienen sufijo
                        st.session_state.orden_cargada_id = df_orden_cargada['ID_Orden'].iloc[0].split('-')[0] + '-' + df_orden_cargada['ID_Orden'].iloc[0].split('-')[1]
            ...
            if 'orden_modificada_df' in st.session_state and not st.session_state.orden_modificada_df.empty:
                ...
                if st.button("💾 Guardar Cambios en la Orden", key="btn_save_changes", type="primary"):
                    ...
                    # Eliminar todas las filas relacionadas con el ID base de la orden
                    df_historico_sin_orden = df_historico_copy[~df_historico_copy['ID_Orden'].str.startswith(orden_id_actual)]
                    
                    # Concatenar el DataFrame original (sin la orden modificada) con el DataFrame editado
                    ...
                    with st.spinner("Guardando cambios en Google Sheets..."):
                        ...
                        if exito:
                            ...
                            st.session_state.orden_modificada_df = edited_orden_df # Actualizar el estado de sesión
                            time.sleep(3)
                            st.cache_data.clear()
                            st.rerun()
                        else:
                ...
                if not edited_orden_df.empty:
                    ...
                    with mod_c2:
                        if celular_contacto:
                            ...
                            st.link_button("📲 Notificar Corrección por WhatsApp", link_wpp, use_container_width=True)
                        else:
